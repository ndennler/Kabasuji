...
public class PuzzleBuilderGUI extends JFrame implements IBuilderGUI{

	/**
	 * 
	 */
	private static final long serialVersionUID = -8141760076950800932L;
	 ...
	 * Create the frame.
	 */
	public PuzzleBuilderGUI(ILevel l, IStock stock, Kabasuji k) {
		level = l;
		this.stock = stock;
		game = k;
		...
		JSpinner movesSpinner = new JSpinner();
		movesSpinner.setFont(new Font("Constantia", Font.PLAIN, 12));
		movesSpinner.setForeground(new Color(105, 105, 105));
		movesSpinner.setModel(new SpinnerNumberModel((int)level.getExtraLogic().get(0), new Integer(1), null, new Integer(1)));
		movesSpinner.setBounds(477, 131, 54, 33);
		contentPane.add(movesSpinner);
		...
		addHintBtn.setBounds(416, 315, 115, 44);
		...
		moveTileBtn.setBounds(416, 370, 115, 44);
		...
		movesSpinner.addChangeListener(new ChangeNumberMovesController(movesSpinner, (PuzzleLevel) level));
	...
	public String getDescription() {
		return "A Puzzle level has a number of moves with which you have to place all the pieces in the board in.";
	}
	...
	public void restore(LevelMemento m) {
		...
		loading.restore(m);
		new PuzzleBuilderGUI(loading, stock, game).setVisible(true);
		this.dispose();
	}
	@Override
	public void newBuilder(ILevel l) {
		new PuzzleBuilderGUI(l, stock, game).setVisible(true);
		this.dispose();
	}
